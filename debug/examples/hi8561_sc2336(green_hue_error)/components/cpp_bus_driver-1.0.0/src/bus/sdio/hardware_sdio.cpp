 ...
 * @LastEditTime: 2025-03-19 09:43:33
 * @License: GPL 3.0
 */
...
{
    bool Hardware_Sdio::begin(int32_t freq_hz, int32_t cs)
    {
        ...
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config _mosi: %d\n", _mosi);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config _sclk: %d\n", _sclk);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config _miso: %d\n", _miso);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config cs: %d\n", cs);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config _port: %d\n", _port);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config _mode: %d\n", _mode);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "hardware_sdio config freq_hz: %d hz\n", freq_hz);
    ...
    bool Hardware_Sdio::write(const void *data, size_t byte)
    {
    ...
    bool Hardware_Sdio::read(void *data, size_t byte)
    {
        spi_transaction_t buffer =
            {
                .flags = static_cast<uint32_t>(NULL),
    ...
    bool Hardware_Sdio::write_read(const void *write_data, void *read_data, size_t data_byte)
    {
        spi_transaction_t buffer =
            {
                .flags = static_cast<uint32_t>(NULL),
