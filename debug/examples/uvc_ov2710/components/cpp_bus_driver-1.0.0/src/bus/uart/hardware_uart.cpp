 ...
 * @LastEditTime: 2025-05-07 09:05:08
 * @License: GPL 3.0
 */
...
namespace Cpp_Bus_Driver
{
    ...
    bool Hardware_Uart::read(void *data, uint32_t length)
    {
        ...
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_read_bytes fail((uart_read_bytes = %d) != length)\n", buffer_size);
            return false;
        }
    ...
    bool Hardware_Uart::write(const void *data, size_t length)
    {
        ...
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_write_bytes fail((uart_write_bytes = %d) != length)\n", buffer_size);
            return false;
        }
