 ...
 * @LastEditTime: 2025-05-07 16:49:25
 * @License: GPL 3.0
 */
...
namespace Cpp_Bus_Driver
{
    bool Hardware_Sdio::begin(int32_t freq_hz)
    {
        if (freq_hz == DEFAULT_CPP_BUS_DRIVER_VALUE)
        {
            freq_hz = DEFAULT_CPP_BUS_DRIVER_SDIO_FREQ_HZ;
                 ...
                 (freq_hz != SDMMC_FREQ_52M) || (freq_hz != SDMMC_FREQ_26M) || (freq_hz != SDMMC_FREQ_DDR50) || (freq_hz != SDMMC_FREQ_SDR50))
        {
            freq_hz = DEFAULT_CPP_BUS_DRIVER_SDIO_FREQ_HZ;
        }
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_host_init fail (error code: 0x%02X)\n", assert);
            return false;
        }
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_host_init_slot fail (error code: 0x%02X)\n", assert);
            // return false;
        }

        _sdio_handle = std::make_unique<sdmmc_card_t>();
        ...
        while (1)
        {
            ...
            {
                assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_card_init fail (error code: 0x%02X)\n", assert);
                return false;
            }
            delay_ms(100);
        }
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_enable_int fail (error code: 0x%02X)\n", assert);
            // return false;
        }
    ...
    bool Hardware_Sdio::wait_interrupt(uint32_t timeout_ms)
    {
        ...
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_wait_int fail (error code: 0x%02X)\n", assert);
            return false;
        }
    ...
    bool Hardware_Sdio::read(uint32_t function, uint32_t write_c32, void *data, size_t byte)
    {
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_read_bytes fail (error code: 0x%02X)\n", assert);
            return false;
        }
    ...
    bool Hardware_Sdio::read(uint32_t function, uint32_t write_c32, uint8_t *data)
    {
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_read_byte fail (error code: 0x%02X)\n", assert);
            return false;
        }
    ...
    bool Hardware_Sdio::read_block(uint32_t function, uint32_t write_c32, void *data, size_t byte)
    {
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_read_blocks fail (error code: 0x%02X)\n", assert);
            return false;
        }
    ...
    bool Hardware_Sdio::write(uint32_t function, uint32_t write_c32, const void *data, size_t byte)
    {
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_write_bytes fail (error code: 0x%02X)\n", assert);
            return false;
        }
    ...
    bool Hardware_Sdio::write(uint32_t function, uint32_t write_c32, uint8_t data, uint8_t *read_d8_verify)
    {
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_write_byte fail (error code: 0x%02X)\n", assert);
            return false;
        }
    ...
    bool Hardware_Sdio::write_block(uint32_t function, uint32_t write_c32, const void *data, size_t byte)
    {
        ...
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "sdmmc_io_write_blocks fail (error code: 0x%02X)\n", assert);
            return false;
        }
