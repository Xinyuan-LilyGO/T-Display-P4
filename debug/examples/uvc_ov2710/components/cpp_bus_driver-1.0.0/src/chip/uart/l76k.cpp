 ...
 * @LastEditTime: 2025-05-07 16:38:26
 * @License: GPL 3.0
 */
#include "l76k.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool L76k::get_info_data(std::shared_ptr<uint8_t[]> &data, uint32_t *length, uint32_t max_length, uint8_t timeout_count)
    {
        ...
        while (1)
        {
            ...
            if (buffer_lenght > 0)
            {
                ...
                if (_bus->read(data.get(), buffer_lenght) == false)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "read fail\n");
                    data = nullptr;
                    *length = 0;
                ...
                assert_log(Log_Level::DEBUG, __FILE__, __LINE__, "buffer_lenght: %d\n",buffer_lenght);
    ...
    bool L76k::parse_rmc_info(std::shared_ptr<uint8_t[]> data, uint32_t length, Rmc &rmc)
    {
        size_t buffer_index = 0;
    ...
    bool L76k::parse_gga_info(std::shared_ptr<uint8_t[]> data, uint32_t length, Gga &gga)
    {
        size_t buffer_index = 0;
