 ...
 * @LastEditTime: 2025-05-08 17:45:34
 * @License: GPL 3.0
 */
...
namespace Cpp_Bus_Driver
{
    ...
    {
    private:
#define SX126X_BUSY_PIN_TIMEOUT_COUNT 10000
#define SX126X_BUSY_FUNCTION_TIMEOUT_COUNT 100
#define SX126X_MAX_TRANSMIT_BUFFER_SIZE 255
    ...
    public:
        ...
        /**
         * @brief 将设备设置为电流消耗最低的睡眠模式（SLEEP 模式），该命令只能在待机模式（STDBY_RC 或 STDBY_XOSC）下发送，
         * 在 NSS 的上升沿之后，除了备份电源调节器（如果需要）和参数 sleepConfig 中指定的模块外，所有模块都将被关闭，开启睡眠
         * 后需要使用set_standby()函数进行唤醒或者使用rtc唤醒
