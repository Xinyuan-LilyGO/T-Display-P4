 ...
 * @LastEditTime: 2025-03-27 14:38:32
 * @License: GPL 3.0
 */
#include "esp_at.h"
...
namespace Cpp_Bus_Driver
{
    ...
    void Esp_At::assert_connect(bool status)
    {
        if (status == true)
        {
            _connect.error_count--;
        ...
        {
            _connect.error_count++;
            if (_connect.error_count > ESP_AT_CONNECT_ERROR_COUNT)
            {
                _connect.error_count = ESP_AT_CONNECT_ERROR_COUNT;
                _connect.status = false;
            }
        }
    }
    ...
    uint32_t Esp_At::get_irq_flag(void)
    {
        ...
        {
            // assert_log(Log_Level::CHIP, __FILE__, __LINE__, "esp_at connect fail\n");
            return false;
        }
