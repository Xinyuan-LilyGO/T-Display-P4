...
static sdio_err_t esp_slave_init_io(void)
{
    ...
    err = sdio_driver_write_byte(0, SD_IO_CCCR_FN_ENABLE, 6, NULL);
    err = sdio_driver_write_byte(0, SD_IO_CCCR_FN_READY, 6, NULL);
    err = sdio_driver_write_byte(0, SD_IO_CCCR_INT_ENABLE, 7, NULL);
    err = sdio_driver_write_byte(0, SD_IO_CCCR_BLKSIZEL, 0, NULL);
    err = sdio_driver_write_byte(0, SD_IO_CCCR_BLKSIZEH, 2, NULL);
    err = sdio_driver_write_byte(0, 0x110, 0, NULL);
    err = sdio_driver_write_byte(0, 0x111, 2, NULL);
    err = sdio_driver_write_byte(0, 0x210, 0, NULL);
    err = sdio_driver_write_byte(0, 0x210, 2, NULL);
    ...
    while (1) {
        ...
        if (num * block_size < length) {
            if (!--cnt) {
                ...
                SDIO_LOGI(TAG, "buffer is not enough: %lu, %d required. Retry...", num, buffer_used);
            }
            if (cnt % 1000 == 0) {
