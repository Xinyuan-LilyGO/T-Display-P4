 ...
 * @LastEditTime: 2025-02-28 17:59:37
 * @License: GPL 3.0
 */
#include "Sx126x.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool Sx126x::begin(int32_t frequency)
    {
        ...
        {
            pin_mode(_busy, Pin_Mode::INPUT, Pin_Status::PULLDOWN);
        }
    ...
    bool Sx126x::check_busy(void)
    {
        if (_busy != DEFAULT_CPP_BUS_DRIVER_VALUE)
        {
            ...
            while (1)
            {
                ...
                timeout_count++;
                if (timeout_count > SX126X_DEFAULT_BUSY_PIN_TIMEOUT_COUNT)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "busy timeout\n");
                    return false;
                }
            }
        }
        else if (_busy_wait_callback != nullptr)
        {
            ...
            while (1)
            {
                ...
                timeout_count++;
                if (timeout_count > SX126X_DEFAULT_BUSY_FUNCTION_TIMEOUT_COUNT)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "busy timeout\n");
                    return false;
                }
    ...
    bool Sx126x::assert_iqr_status(uint16_t assert_status, Iqr_Status &status)
    {
        if (assert_status == static_cast<uint8_t>(-1))
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "read error\n");
            return false;
        }

        status.all_flag.tx_done = assert_status & 0B0000000000000001;
        status.all_flag.rx_done = (assert_status & 0B0000000000000010) > 1;
        status.all_flag.preamble_detected = (assert_status & 0B0000000000000100) > 2;
        status.gfsk_flag.sync_word_valid = (assert_status & 0B0000000000001000) > 3;
        status.lora_reg_flag.header_valid = (assert_status & 0B0000000000010000) > 4;
        status.lora_reg_flag.header_error = (assert_status & 0B0000000000100000) > 5;
        status.all_flag.crc_error = (assert_status & 0B0000000001000000) > 6;
        status.lora_reg_flag.cad_done = (assert_status & 0B0000000010000000) > 7;
        status.lora_reg_flag.cad_detected = (assert_status & 0B0000000100000000) > 8;
        status.all_flag.tx_rx_timeout = (assert_status & 0B0000001000000000) > 9;
        status.lrfhss_flag.pa_ramped_up_hop = (assert_status & 0B0100000000000000) > 14;
    ...
    {
        uint32_t buffer_freq = (freq * (static_cast<uint32_t>(1) << 25)) / 32.0;
    ...
    bool Sx126x::set_frequency(float freq)
    {
        ...
        _param.frequency = freq;

        return true;
    }

    bool Sx126x::config_lora_params(float frequency, Lora_Bw bw, float current_limit, int8_t power, Sf sf, Cr cr, Lora_Crc_Type crc_type,
                                    uint16_t preamble_length, uint16_t sync_word, Regulator_Mode regulator_mode)
    {
        ...
        // 设置包的参数
        if (set_lora_packet_params(preamble_length, Lora_Header_Type::VARIABLE_LENGTH_PACKET, 255, crc_type, Invert_Iq::STANDARD_IQ_SETUP) == false)
        {
    ...
    bool Sx126x::start_lora_transmit(Chip_Mode chip_mode, Interrupt_Pin int_pin, Fallback_Mode fallback_mode,
                                     uint32_t time_out_us, uint16_t preamble_length)
    {
        // 从RX或TX模式退出返回的模式设定
        ...
        case Chip_Mode::RX:
            ...
            case Interrupt_Pin::DIO1:
                ...
                break;
            case Interrupt_Pin::DIO2:
                if (set_dio_irq_params(0B0000001001100010, 0, 0B0000000000000010, 0) == false)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_dio_irq_params fail\n");
                    return false;
                }
                break;
            case Interrupt_Pin::DIO3:
                if (set_dio_irq_params(0B0000001001100010, 0, 0, 0B0000000000000010) == false)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_dio_irq_params fail\n");
                    return false;
                }
        ...
        case Chip_Mode::TX:
            ...
            {
            case Interrupt_Pin::DIO1:
                if (set_dio_irq_params(0B0000001000000001, 0B0000000000000001, 0, 0) == false)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_dio_irq_params fail\n");
                    return false;
                }
                break;
            case Interrupt_Pin::DIO2:
                if (set_dio_irq_params(0B0000001000000001, 0, 0B0000000000000001, 0) == false)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_dio_irq_params fail\n");
                    return false;
                }
                break;
            case Interrupt_Pin::DIO3:
                if (set_dio_irq_params(0B0000001000000001, 0, 0, 0B0000000000000001) == false)
                {
                    assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_dio_irq_params fail\n");
                    return false;
                }
    ...
    uint8_t Sx126x::receive_data(uint8_t *data, uint8_t length)
    {
        ...
        // // 检查中断
        // Iqr_Status buffer_is;
        // if (assert_iqr_status(get_irq_status(), buffer_is) == false)
        // {
        //     assert_log(Log_Level::CHIP, __FILE__, __LINE__, "assert_Iqr_status fail\n");
        //     _assert = 1;
        //     return false;
        // }
        // else
    ...
    bool Sx126x::config_gfsk_params(float frequency, uint32_t br, Gfsk_Bw bw, float current_limit, int8_t power, float freq_deviation,
                                    uint8_t *sync_word, uint8_t sync_word_length, Pulse_Shape ps, Sf sf, Gfsk_Crc_Type crc_type,
                                    uint16_t crc_initial, uint16_t crc_polynomial, uint16_t preamble_length, Regulator_Mode regulator_mode)
    {
        // 启用13MHz晶振模式
        if (set_standby(Stdby_Config::STDBY_RC) == false)
        {
        ...
        {
            _param.gfsk.preamble_detector = Preamble_Detector::LENGTH_32BITS;
        }
        else if (buffer_preamble_detector >= 24)
        {
            _param.gfsk.preamble_detector = Preamble_Detector::LENGTH_24BITS;
        }
        else if (buffer_preamble_detector >= 16)
        {
            _param.gfsk.preamble_detector = Preamble_Detector::LENGTH_16BITS;
        }
        else if (buffer_preamble_detector > 0)
        {
            _param.gfsk.preamble_detector = Preamble_Detector::LENGTH_8BITS;
        }
        else
        {
        ...
        if (set_gfsk_packet_params(preamble_length, _param.gfsk.preamble_detector, sync_word_length * 8, Addr_Comp::FILTERING_DISABLE,
                                   Gfsk_Header_Type::VARIABLE_PACKET, 255, crc_type, Whitening::NO_ENCODING) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_gfsk_packet_params fail\n");
            return false;
        }
    ...
    bool Sx126x::start_gfsk_transmit(Chip_Mode chip_mode, Interrupt_Pin int_pin, Fallback_Mode fallback_mode, uint32_t time_out_us, uint16_t preamble_length)
    {
        // 从RX或TX模式退出返回的模式设定
        if (set_rx_tx_fallback_mode(fallback_mode) == false)
        {
