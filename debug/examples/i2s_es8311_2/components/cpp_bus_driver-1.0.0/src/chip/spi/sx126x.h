 ...
 * @LastEditTime: 2025-02-28 17:34:09
 * @License: GPL 3.0
 */
...
namespace Cpp_Bus_Driver
{
    ...
    private:
#define SX126X_DEFAULT_BUSY_PIN_TIMEOUT_COUNT 10000
#define SX126X_DEFAULT_BUSY_FUNCTION_TIMEOUT_COUNT 100
    ...
    public:
         ...
         * @param assert_status 断言状态语句，由get_irq_status()函数获取
         * @param &status 使用Iqr_Status结构体配置，相应位自动置位
         * @return
         * @Date 2025-02-13 13:33:15
         */
        bool assert_iqr_status(uint16_t assert_status, Iqr_Status &status);
         ...
         */
        bool config_lora_params(float frequency, Lora_Bw bw, float current_limit, int8_t power, Sf sf = Sf::SF9, Cr cr = Cr::CR_4_7,
         ...
         * @param int_pin 使用 Interrupt_Pin:: 配置，中断输出引脚
         * @param fix_bw_500khz_sensitivity_enable [true]：开启修复，[false]：关闭修复，修复LORA发送模式在带宽为500khz下灵敏度衰减问题（SX126x手册第15.1节），只有在LORA模式并且带宽为500khz才开启
         * @param time_out_us 超时时间 = 设置的超时时间 * 15.625μs，设置的超时时间最大值为 16777215 （0xFFFFFF）。
         ...
         */
        bool start_lora_transmit(Chip_Mode chip_mode, Interrupt_Pin int_pin = Interrupt_Pin::DIO1, Fallback_Mode fallback_mode = Fallback_Mode::STDBY_XOSC,
                                 uint32_t time_out_us = 0xFFFFFF, uint16_t preamble_length = 8);
         ...
         * @brief 接收数据
         * @param *data 接收数据的指针
         * @param length 接收数据的长度，如果等于0将默认接收对应接收到的数据长度并返回接受到的长度数据
         * @return 接收的数据长度，如果接收错误或者接收长度为0都返回0
    ...
    private:
        struct Param
        {
            ...
            struct
            {
                ...
                Preamble_Detector preamble_detector = Preamble_Detector::LENGTH_8BITS;
    ...
    public:
         ...
         * @Date 2025-02-19 09:17:55
         */
        bool config_gfsk_params(float frequency, uint32_t br, Gfsk_Bw bw, float current_limit, int8_t power, float freq_deviation = 10.0,
                                ...
                                uint16_t preamble_length = 32, Regulator_Mode regulator_mode = Regulator_Mode::LDO_AND_DCDC);
         ...
         * @Date 2025-02-19 09:17:55
         */
        bool start_gfsk_transmit(Chip_Mode chip_mode, Interrupt_Pin int_pin = Interrupt_Pin::DIO1, Fallback_Mode fallback_mode = Fallback_Mode::STDBY_XOSC,
                                 uint32_t time_out_us = 0xFFFFFF, uint16_t preamble_length = 32);
