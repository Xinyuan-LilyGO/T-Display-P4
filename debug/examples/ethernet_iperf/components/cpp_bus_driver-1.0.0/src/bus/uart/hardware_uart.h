 ...
 * @LastEditTime: 2025-02-17 11:18:04
 * @License: GPL 3.0
 */
#pragma once
...
namespace Cpp_Bus_Driver
{
    ...
    {
    private:
#define UART_RX_MAX_SIZE (1024 * 2)
    ...
    public:
        Hardware_Uart(int32_t tx, int32_t rx, int8_t port = DEFAULT_CPP_BUS_DRIVER_VALUE)
            : _tx(tx), _rx(rx), _port(port)
        {
        }

        bool begin(int32_t baud_rate = DEFAULT_CPP_BUS_DRIVER_VALUE) override;
        bool read(void *data, uint32_t length) override;
