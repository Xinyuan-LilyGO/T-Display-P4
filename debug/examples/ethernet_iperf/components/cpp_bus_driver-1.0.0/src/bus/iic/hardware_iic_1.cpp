 ...
 * @LastEditTime: 2025-03-12 18:05:35
 * @License: GPL 3.0
 */
...
{
    bool Hardware_Iic_1::begin(int32_t frequency, int16_t address)
    {
        ...
        if (_iic_bus == nullptr)
        {
            ...
            assert = i2c_new_master_bus(&bus_config, &_iic_bus);
            if (assert != ESP_OK)
            {
        ...
        if (_iic_device == nullptr)
        {
            const i2c_device_config_t device_config =
                {
                    .dev_addr_length = I2C_ADDR_BIT_LEN_7,
                    .device_address = static_cast<uint16_t>(address),
                    .scl_speed_hz = static_cast<uint32_t>(frequency),
                    .scl_wait_us = 0,
                    .flags =
                        {
                            .disable_ack_check = 1,
    ...
    {
        esp_err_t assert = i2c_master_receive(_iic_device, data, length, DEFAULT_CPP_BUS_DRIVER_IIC_WAIT_TIMEOUT_MS);
        if (assert != ESP_OK)
        {
    ...
    {
        esp_err_t assert = i2c_master_transmit(_iic_device, data, length, DEFAULT_CPP_BUS_DRIVER_IIC_WAIT_TIMEOUT_MS);
        if (assert != ESP_OK)
        {
    ...
    {
        esp_err_t assert = i2c_master_transmit_receive(_iic_device, write_data, write_length, read_data, read_length, DEFAULT_CPP_BUS_DRIVER_IIC_WAIT_TIMEOUT_MS);
        if (assert != ESP_OK)
        {
    ...
    {
        esp_err_t assert = i2c_master_probe(_iic_bus, address, DEFAULT_CPP_BUS_DRIVER_IIC_WAIT_TIMEOUT_MS);
        if (assert != ESP_OK)
        {
