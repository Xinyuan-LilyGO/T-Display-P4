 ...
 * @LastEditTime: 2025-03-14 16:13:08
 * @License: GPL 3.0
 */
#include "es8311.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool Es8311::begin(int32_t frequency)
    {
    ...
    bool Es8311::software_reset(void)
    {
        uint8_t buffer = 0;

        if (Iic_Guide::_bus->read(static_cast<uint8_t>(Cmd::RW_RESET_CONTROL), &buffer) == false)
        {
            Iic_Guide::assert_log(Log_Level::CHIP, __FILE__, __LINE__, "read fail\n");
            return false;
        }
        buffer = (buffer & 0B11100000) | 0B00011111;
        if (Iic_Guide::_bus->write(static_cast<uint8_t>(Cmd::RW_RESET_CONTROL), buffer) == false)
        {
    ...
    bool Es8311::set_chip_current_status(bool enable)
    {
        uint8_t buffer = 0;
        ...
        if (enable == true)
        {
            buffer = (buffer & 0B01111111) | 0B10000000;
        }
        else
        {
            buffer &= 0B01111111;
    ...
    bool Es8311::set_sdp_data_bit_length(Sdp dsp, Data_Bit_Length length)
    {
        uint8_t buffer = 0;
