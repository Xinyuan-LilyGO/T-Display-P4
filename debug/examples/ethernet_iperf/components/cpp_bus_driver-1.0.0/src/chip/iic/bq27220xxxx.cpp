 ...
 * @LastEditTime: 2025-03-14 09:25:53
 * @License: GPL 3.0
 */
...
namespace Cpp_Bus_Driver
{
    ...
    bool Bq27220xxxx::begin(int32_t frequency)
    {
        ...
        if (buffer != static_cast<uint8_t>(BQ27220YZFR_DEVICE_ID))
        {
    ...
    uint16_t Bq27220xxxx::get_standby_current(void)
    {
        uint8_t buffer[2] = {0};
    ...
    uint16_t Bq27220xxxx::get_max_load_current(void)
    {
        uint8_t buffer[2] = {0};
    ...
    uint16_t Bq27220xxxx::get_state_of_charge(void)
    {
        uint8_t buffer[2] = {0};

        if (_bus->read(static_cast<uint8_t>(Cmd::RO_STATE_OF_CHARGE_START), buffer, 2) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "read fail\n");
            return -1;
        }
    ...
    uint16_t Bq27220xxxx::get_state_of_health(void)
    {
        uint8_t buffer[2] = {0};

        if (_bus->read(static_cast<uint8_t>(Cmd::RO_STATE_OF_HEALTH_START), buffer, 2) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "read fail\n");
            return -1;
        }
