/*
 * @Description: None
 * @Author: LILYGO_L
 * @Date: 2025-02-13 15:26:23
 * @LastEditTime: 2025-02-17 11:34:55
 * @License: GPL 3.0
 */
#include "hardware_uart.h"

namespace Cpp_Bus_Driver
{
    bool Hardware_Uart::begin(int32_t baud_rate)
    {
        _baud_rate = (baud_rate == DEFAULT_CPP_BUS_DRIVER_VALUE) ? DEFAULT_CPP_BUS_DRIVER_UART_BAUD_RATE : baud_rate;

        if ((_port < UART_NUM_0) || (_port > UART_NUM_MAX))
        {
            _port = UART_NUM_1;
        }

        assert_log(Log_Level::INFO, __FILE__, __LINE__, "configuring iic_port: %d\n", _port);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "configuring sda_io_num: %d\n", _tx);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "configuring scl_io_num: %d\n", _rx);
        assert_log(Log_Level::INFO, __FILE__, __LINE__, "configuring scl_speed_hz: %d bps\n", _baud_rate);

        const uart_config_t uart_config =
            {
                .baud_rate = _baud_rate,
                .data_bits = UART_DATA_8_BITS,
                .parity = UART_PARITY_DISABLE,
                .stop_bits = UART_STOP_BITS_1,
                .flow_ctrl = UART_HW_FLOWCTRL_DISABLE,
                .rx_flow_ctrl_thresh = 122,
                .source_clk = UART_SCLK_DEFAULT,
                .flags =
                    {
                        .allow_pd = 1,
                        .backup_before_sleep = 1,
                    },
            };

        // We won't use a buffer for sending data.
        esp_err_t assert = uart_driver_install(static_cast<uart_port_t>(_port), UART_RX_MAX_SIZE, 0, 0, NULL, 0);
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_driver_install fail (error code: %#X)\n", assert);
            return false;
        }

        assert = uart_param_config(static_cast<uart_port_t>(_port), &uart_config);
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_param_config fail (error code: %#X)\n", assert);
            return false;
        }

        assert = uart_set_pin(static_cast<uart_port_t>(_port), _tx, _rx, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
        if (assert != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_set_pin fail (error code: %#X)\n", assert);
            return false;
        }

        return true;
    }

    bool Hardware_Uart::read(void *data, uint32_t length)
    {
        if (uart_read_bytes(static_cast<uart_port_t>(_port), data, length, pdMS_TO_TICKS(DEFAULT_CPP_BUS_DRIVER_UART_WAIT_TIMEOUT_MS)) == (-1))
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_read_bytes fail\n");
            return false;
        }
        return true;
    }

    bool Hardware_Uart::write(const void *data, size_t length)
    {
        if (uart_write_bytes(static_cast<uart_port_t>(_port), data, length) == (-1))
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_read_bytes fail\n");
            return false;
        }
        return true;
    }

    size_t Hardware_Uart::get_rx_buffer_length(void)
    {
        size_t buffer = 0;

        if (uart_get_buffered_data_len(static_cast<uart_port_t>(_port), &buffer) != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_read_bytes fail\n");
            return false;
        }

        return buffer;
    }

    bool Hardware_Uart::clear_rx_buffer_data(void)
    {
        if (uart_flush_input(static_cast<uart_port_t>(_port)) != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_flush_input fail\n");
            return false;
        }

        return true;
    }

    bool Hardware_Uart::set_baud_rate(uint32_t baud_rate)
    {
        if (uart_set_baudrate(static_cast<uart_port_t>(_port), baud_rate) != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_set_baudrate fail\n");
            return false;
        }

        return true;
    }

    uint32_t Hardware_Uart::get_baud_rate(void)
    {
        uint32_t buffer = 0;

        if (uart_get_baudrate(static_cast<uart_port_t>(_port), &buffer) != ESP_OK)
        {
            assert_log(Log_Level::BUS, __FILE__, __LINE__, "uart_set_baudrate fail\n");
            return -1;
        }

        return buffer;
    }
}