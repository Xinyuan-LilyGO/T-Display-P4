 ...
 * @LastEditTime: 2025-03-19 09:46:17
 * @License: GPL 3.0
 */
#include "Sx126x.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool Sx126x::assert_iqr_status(uint16_t irq_flag, Iqr_Status &status)
    {
                                    ...
                                    uint16_t preamble_length, uint16_t sync_word, Regulator_Mode regulator_mode)
    {
        ...
        // 清除所有中断
        if (clear_irq_flag(Irq_Flag::ALL) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "clear_irq_flag fail\n");
            return false;
        }
        ...
        // 设置包的参数
        if (set_lora_packet_params(preamble_length, Lora_Header_Type::VARIABLE_LENGTH_PACKET, SX126X_DEFAULT_MAX_TRANSMIT_BUFFER_SIZE, crc_type, Invert_Iq::STANDARD_IQ_SETUP) == false)
        {
    ...
    bool Sx126x::start_lora_transmit(Chip_Mode chip_mode, Fallback_Mode fallback_mode,
    ...
    uint8_t Sx126x::receive_data(uint8_t *data, uint8_t length)
    {
        ...
        // if (assert_iqr_flag(get_irq_flag(), buffer_is) == false)
        // {
        //     assert_log(Log_Level::CHIP, __FILE__, __LINE__, "assert_iqr_flag fail\n");
        //     _assert = 1;
        //     return false;
        // }
        // else
                                    ...
                                    uint16_t crc_initial, uint16_t crc_polynomial, uint16_t preamble_length, Regulator_Mode regulator_mode)
    {
        ...
        if (set_gfsk_packet_params(preamble_length, _param.gfsk.preamble_detector, sync_word_length * 8, Addr_Comp::FILTERING_DISABLE,
                                   Gfsk_Header_Type::VARIABLE_PACKET, SX126X_DEFAULT_MAX_TRANSMIT_BUFFER_SIZE, crc_type, Whitening::NO_ENCODING) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "set_gfsk_packet_params fail\n");
            return false;
        }
    ...
    bool Sx126x::start_gfsk_transmit(Chip_Mode chip_mode, Fallback_Mode fallback_mode, uint32_t time_out_us, uint16_t preamble_length)
    {
        // 从RX或TX模式退出返回的模式设定
        if (set_rx_tx_fallback_mode(fallback_mode) == false)
        {
    ...
    {
        std::shared_ptr<uint8_t[]> buffer = std::make_shared<uint8_t[]>(SX126X_DEFAULT_MAX_TRANSMIT_BUFFER_SIZE);

        memset(buffer.get(), 0, SX126X_DEFAULT_MAX_TRANSMIT_BUFFER_SIZE);

        if (write_buffer(buffer.get(), SX126X_DEFAULT_MAX_TRANSMIT_BUFFER_SIZE, 0) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "write_buffer fail\n");
            return false;
        }
