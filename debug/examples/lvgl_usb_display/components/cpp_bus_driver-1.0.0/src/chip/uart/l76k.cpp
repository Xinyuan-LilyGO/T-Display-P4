 ...
 * @LastEditTime: 2025-03-15 18:01:29
 * @License: GPL 3.0
 */
#include "l76k.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool L76k::get_device_id(size_t *search_index)
    {
        ...
        if (search(buffer_cmd, std::strlen(buffer_cmd), buffer.get(), buffer_lenght, search_index) == false)
        {
    ...
    bool L76k::parse_rmc_info(std::shared_ptr<uint8_t[]> data, size_t length, Rmc &rmc)
    {
        ...
        if (search(buffer_cmd, std::strlen(buffer_cmd), data.get(), length, &buffer_index) == false)
        {
            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "search fail\n");
        }
        else
            ...
            for (auto i = buffer_index + std::strlen(buffer_cmd); i < length; i++)
            {
                ...
                else
                {
                    ...
                    case 8: // <HDOP>
                    {
                        ...
                        if (search(",", 1, data.get() + i, length - i, &buffer_index) == false) // 搜索下一个
                        {
                            assert_log(Log_Level::CHIP, __FILE__, __LINE__, "search fail\n");
                        }
