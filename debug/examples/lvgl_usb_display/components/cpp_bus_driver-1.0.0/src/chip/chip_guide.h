 ...
 * @LastEditTime: 2025-03-12 15:51:16
 * @License: GPL 3.0
 */
#pragma once
...
namespace Cpp_Bus_Driver
{
    ...
    // class Iic_Iis_Guide : public Tool
    // {
    // protected:
    //     std::shared_ptr<Iic_Guide> _iic_bus;
    //     std::shared_ptr<Iis_Guide> _iis_bus;

    // private:
    //     int16_t _address;

    // public:
    //     Iic_Guide(std::shared_ptr<Bus_Iic_Guide> iic_bus, std::shared_ptr<Bus_Iis_Guide> iis_bus, int16_t iic_address, int16_t address)
    //         : _bus(bus), _address(address)
    //     {
    //     }

    //     virtual bool begin(i2s_mclk_multiple_t iis_mclk_multiple, uint32_t iis_sample_rate_hz, i2s_data_bit_width_t iis_data_bit_width,
    //                        int32_t iic_frequency = DEFAULT_CPP_BUS_DRIVER_VALUE);
    // };
