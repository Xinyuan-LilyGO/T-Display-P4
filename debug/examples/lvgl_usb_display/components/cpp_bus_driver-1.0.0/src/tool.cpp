 ...
 * @LastEditTime: 2025-03-19 10:26:37
 * @License: GPL 3.0
 */
#include "tool.h"
...
namespace Cpp_Bus_Driver
{
    ...
    bool Tool::search(const char *search_sample, size_t sample_length, const uint8_t *search_library, size_t search_library_length, size_t *search_index)
    {
        // 检查参数有效性
        if (search_sample == nullptr || search_library == nullptr || sample_length == 0 || search_library_length == 0 || sample_length > search_library_length)
        {
            assert_log(Log_Level::INFO, __FILE__, __LINE__, "invalid input parameters\n");
            return false;
        }
        ...
        {
            assert_log(Log_Level::INFO, __FILE__, __LINE__, "search fail\n");
            return false;
        }
